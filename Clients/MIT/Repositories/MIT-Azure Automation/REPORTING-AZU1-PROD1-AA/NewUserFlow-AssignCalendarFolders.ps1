...
Mangano IT - New User Flow - Assign New User to Calendar Folder
...
$TicketNoteDetails = ''
$FailedNoteDetails = ''
$TaskNotes = 'Grant Access to Calendar'
[string]$Log = ''
...
foreach ($Task in $Tasks) {
    if (!$Task.closedFlag -and $Task.notes -eq $TaskNotes) {
        ...
        $SharingPermissionFlags = $Resolution.SharingPermissionFlags
        $Arguments = @{
            TenantUrl = $TenantUrl
        ...
        $Operation = .\EXO-AssignCalendar.ps1 @Arguments | ConvertFrom-Json
...
# Add note if the user was added to any calendar folders
$Text = $TaskNotes + " [automated task]`n`n"

if ($TicketNoteDetails -ne '' -or $FailedNoteDetails -ne '') {
    if ($TicketNoteDetails -ne '') {
        $Text += "`n`nThe new user has been assigned access to the following calendar folders:$TicketNoteDetails"
    }
    if ($FailedNoteDetails -ne '') {
        $Text += "`n`nThe new user has NOT been assigned access to the following calendar folders:$FailedNoteDetails"
    }
