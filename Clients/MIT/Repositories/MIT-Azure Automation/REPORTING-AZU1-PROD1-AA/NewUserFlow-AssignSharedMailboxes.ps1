...
Mangano IT - New User Flow - Assign New User to Shared Mailboxes
...
$TicketNoteDetails = ''
$FailedNoteDetails = ''
$TaskNotes = 'Grant Access to Shared Mailbox'
[string]$Log = ''
...
# If task is not complete, attempt to assign access to shared mailbox
foreach ($Task in $Tasks) {
    if (!$Task.closedFlag -and $Task.notes -eq $TaskNotes) {
        ...
        if ($AccessRights -eq "SendAs") {
            $Operation = .\EXO-AssignSharedMailboxSendAs.ps1 @Arguments | ConvertFrom-Json
        } else {
            $Operation = .\EXO-AssignSharedMailbox.ps1 @Arguments | ConvertFrom-Json
        }
...
# Add note if the user was added to any shared mailboxes
$Text = $TaskNotes + " [automated task]`n`n"

if ($TicketNoteDetails -ne '' -or $FailedNoteDetails -ne '') {
    if ($TicketNoteDetails -ne '') {
        $Text += "`n`nThe new user has been assigned access to the following shared mailboxes:$TicketNoteDetails"
    }
    if ($FailedNoteDetails -ne '') {
        $Text += "`n`nThe new user has NOT been assigned access to the following shared mailboxes:$FailedNoteDetails"
    }
