...
function ProcessVIPContact {
    ...
    if ($IsVIP -or ($Item.Priority -contains $Ticket.priority.name)) {
        ...
        Write-MessageLog -Message "AH conditions NOT met for TimeEntry ID: $($TimeEntry.id). No changes will be made."
    }
}
...
function UpdateTimeEntry {
    ...
    $currentOption = $TimeEntry.billableOption

    if ($currentOption -eq "Billable" -and $BillableOption -eq "DoNotBill") {
        Write-MessageLog -Message "Updating billable option from 'Billable' to 'DoNotBill' for TimeEntry ID: $($TimeEntry.id)."
        ...
        Write-MessageLog -Message "No update needed for TimeEntry ID: $($TimeEntry.id). Current option: '$currentOption', Target: '$BillableOption'."
    }
}
