...
Updated by: Joshua Ceccato
Version: 1.0

This runbook is designed to be run in conjunction with a Power Automate flow.
Updated to include extension attributes to store user types, service, standard, basic - for MSA/billing purposes.
...
$ApiUrl = "https://graph.microsoft.com/beta/users?`$select=id,displayName,userPrincipalName,jobTitle,companyName,signInActivity,userType,assignedLicenses,streetAddress,city,state,postalCode,accountEnabled,department,onPremisesExtensionAttributes"
$Result = @()
while ($Null -ne $ApiUrl) { # Perform pagination if next page link (odata.nextlink) returned.
    try {
        ...
        $ApiUrl = "https://graph.microsoft.com/beta/users?`$select=id,displayName,userPrincipalName,jobTitle,companyName,userType,assignedLicenses,streetAddress,city,state,postalCode,accountEnabled,department,onPremisesExtensionAttributes"
        $Response = Invoke-WebRequest -UseBasicParsing -Method GET -Uri $ApiUrl -ContentType "application\json" -Headers $WebRequestHeaders | ConvertFrom-Json
    }
    ...
    if ($Response.value) {
        ...
        foreach ($User in $Users) {
            ...
            $Result += New-Object PSObject -property $([ordered]@{ 
                ...
                extensionAttribute15 = if ($User.onPremisesExtensionAttributes.extensionAttribute15) { $User.onPremisesExtensionAttributes.extensionAttribute15 } else {""}
            })
        }
    }
