...
Mangano IT - Exchange Online - Assign User Rights for Shared Mailbox (Send As)
...
param(
    ...
    [Parameter(Mandatory=$true)][string]$SharedMailbox,
...
if ($null -ne $Mailbox) {
    try {
        $Log += "Assigning $EmailAddress access to $SharedMailbox with rights $AccessRights...`n"
        Add-RecipientPermission -Identity $SharedMailbox -Trustee $EmailAddress -AccessRights $AccessRights | Out-Null
        $Log += "SUCCESS: Assigned $EmailAddress access to $SharedMailbox with rights $AccessRights."
        Write-Warning "SUCCESS: Assigned $EmailAddress access to $SharedMailbox with rights $AccessRights."
        $Result = $true
    }
    catch {
        $Log += "ERROR: Unable to assign $EmailAddress access to $SharedMailbox with rights $AccessRights.`nERROR DETAILS: " + $_
        Write-Error "Unable to assign $EmailAddress access to $SharedMailbox with rights $AccessRights : $_"
        $Result = $false
    }
}
