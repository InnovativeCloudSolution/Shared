...
Mangano IT - Azure Active Directory - Remove User from Group (AAD)
...
try {
    ...
    $Log += "INFO: $UserId is a member of $SecurityGroupId.`n"
    Write-Warning "INFO: $UserId is a member of $SecurityGroupId."
} catch {
...
$ApiArguments = @{
    Uri = "https://graph.microsoft.com/v1.0/groups/$SecurityGroupId/members/$UserId" + '/$ref'
    Method = 'DELETE'
    Headers = @{ 'Content-Type'="application/json";'Authorization'="Bearer $BearerToken";'ConsistencyLevel'='eventual' }
    UseBasicParsing = $true
}
...
# Contacts Graph API to remove member from group - only runs if user was found
if ($null -ne $CheckUserResponse) {
    try {
        $ApiResponse = Invoke-WebRequest @ApiArguments
    } catch {
        $Result = $false
        $Log += "ERROR: Unable to remove $UserId from $SecurityGroupId.`nERROR DETAILS:" + $_
        Write-Error "Unable to remove $UserId from $SecurityGroupId : $_"
    }
    if ($ApiResponse.StatusCode -eq 204) { 
        $Log += "SUCCESS: $UserId has been removed from $SecurityGroupId."
        Write-Warning "SUCCESS: $UserId has been removed from $SecurityGroupId."
        $Result = $true 
    }
} else {
    $Log += "INFO: $UserId is already not a member of $SecurityGroupId."
    $Result = $true 
}
