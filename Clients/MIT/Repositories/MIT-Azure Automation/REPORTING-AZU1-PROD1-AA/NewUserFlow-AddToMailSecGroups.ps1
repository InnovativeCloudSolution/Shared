...
Mangano IT - New User Flow - Assign New User to Mail-Enabled Security Groups
...
$TicketNoteDetails = ''
$FailedNoteDetails = ''
$TaskNotes = 'Add to Mail-Enabled Security Group'
[string]$Log = ''
...
# If task is not complete, attempt to assign access to mail-enabled security group
...
# Add note if the user was added to any mail-enabled groups
$Text = $TaskNotes + " [automated task]`n`n"

if ($TicketNoteDetails -ne '' -or $FailedNoteDetails -ne '') {
    if ($TicketNoteDetails -ne '') {
        $Text += "`n`nThe new user has been assigned access to the following mail-enabled security groups:$TicketNoteDetails"
    }
    if ($FailedNoteDetails -ne '') {
        $Text += "`n`nThe new user has NOT been assigned access to the following mail-enabled security groups:$FailedNoteDetails"
    }
