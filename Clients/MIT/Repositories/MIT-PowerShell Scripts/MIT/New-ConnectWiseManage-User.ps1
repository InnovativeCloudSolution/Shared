...
if ($null -eq $ContactCheck) {
	...
	try {
		$Contact = New-CWMContact -firstName $FirstName -lastName $LastName -title $Title -company @{id=$CompanyId} -site @{id=$SiteID} -communicationItems $comArray -types $contactArray
