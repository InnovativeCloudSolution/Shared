...
$Log="The parameters that have been provided are:"
...
Try{
    ...
    $MSOLUser = New-MsolUser -TenantId $TenantID -FirstName $FirstName -LastName $LastName -PhoneNumber "07 3151 9000" -UserPrincipalName $Email -DisplayName $DisplayName -City "Newmarket" -StreetAddress "16 Edmondstone Street" -PostalCode "4051" -State "Queensland" -Country "AU" -Title $Title -UsageLocation "AU" -Password $Password -ForceChangePassword $false
...
if($AdminRequired){
    ...
    Try{
        $Log+="`r`nCreating the AD User`r`n"
        $AdminMSOLUser = New-MsolUser -TenantId $TenantID -UserPrincipalName $AdminEmail -DisplayName $AdminDisplayName -MobilePhone $MobilePhone -UsageLocation "AU" -Password $Password -ForceChangePassword $false
        $Log+="AD Account created with Username: $Username `r`n"
...
while ((Get-Mailbox $Email -ErrorAction SilentlyContinue).Name -eq $null){
