...
data_to_log = {}
bot_name = "MIT-EXO - Generate Access Token"
...
def main():
    try:
        try:
            ticket_number = input.get_value("TicketNumber_1757568474679")
        except Exception:
        ...
        access_token = get_access_token(log, http_client, tenant_id, exo_client_id, exo_client_secret, scope="https://outlook.office365.com/.default", log_prefix="EXO")
        if not isinstance(access_token, str) or "." not in access_token:
            record_result(log, ResultLevel.WARNING, "Access token is malformed (missing dots)")
            return

        data_to_log["exo_token"] = access_token
        record_result(log, ResultLevel.SUCCESS, "EXO access token retrieved successfully")
