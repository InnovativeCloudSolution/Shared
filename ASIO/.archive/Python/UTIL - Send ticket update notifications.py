...
def get_company_data_from_ticket(log, http_client, cwpsa_base_url, ticket_number):
    ...
    if response and response.status_code == 200:
        ...
        return company_identifier, company_name
        ...
        log.error(f"Failed to retrieve ticket [{ticket_number}]: No response received")
...
def main():
    try:
        try:
            notification = input.get_value("Notification_1747968620394")
            ...
            provided_token = input.get_value("AccessToken_1746937205103")
        ...
        log.info(f"Raw inputs received: ticket_number=[{ticket_number}], recipient_emails=[{recipient_emails}], first_name=[{first_name}], notification=[{notification}], user_name=[{user_name}], access=[{access}]")

        notification = notification.strip() if notification else ""
        ...
        log.info(f"Processed inputs: ticket_number=[{ticket_number}], recipient_emails=[{recipient_emails}], first_name=[{first_name}], notification=[{notification}], user_name=[{user_name}], access=[{access}]")

        if not notification:
            record_result(log, ResultLevel.INFO, "Bot completed with no notifications selected. No action taken")
            return

        company_name = ""
        if ticket_number:
            company_identifier, company_name = get_company_data_from_ticket(log, http_client, cwpsa_base_url, ticket_number)
        ...
        if notification == "Reviewed":
            log.info("Proceeding with reviewed notification workflow")
            ...
            log.info("Attempting to send reviewed email")
            if send_email(log, http_client, msgraph_base_url, access_token, sender_email, recipient_emails, subject, html_body):
                record_result(log, ResultLevel.SUCCESS, f"Sent user ticket reviewed notification to [{recipient_emails}]")
            else:
                record_result(log, ResultLevel.WARNING, f"Failed to send ticket reviewed notification to [{recipient_emails}]")

        elif notification == "Access Request":
            log.info("Proceeding with access request notification workflow")
            ...
            if send_email(log, http_client, msgraph_base_url, access_token, sender_email, recipient_emails, subject, html_body):
                record_result(log, ResultLevel.SUCCESS, f"Sent access request notification to [{recipient_emails}]")
            else:
                record_result(log, ResultLevel.WARNING, f"Failed to send access request notification to [{recipient_emails}]")

        else:
            record_result(log, ResultLevel.INFO, f"No action defined for notification type [{notification}]. No action taken")
