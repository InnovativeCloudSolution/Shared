...
def get_named_location(log, http_client, msgraph_base_url, location_identifier, token):
    try:
        ...
        if not response or response.status_code != 200:
            log.error(f"Failed to retrieve Conditional Access Named Locations Status: {response.status_code if response else 'N/A'}")
            return []
...
def main():
    try:
        try:
            location_identifier = input.get_value("LocationName_1744579845409")
        ...
        location_identifier = location_identifier.strip() if location_identifier else ""
        ...
        log.info(f"Received input Location = [{location_identifier}], Ticket = [{ticket_number}]")

        if not location_identifier:
            log.error("Location Identifier is empty or invalid")
            log.result_message(ResultLevel.FAILED, "Location Identifier input is required")
            return
        ...
        named_locations = get_named_location(log, http_client, msgraph_base_url, location_identifier, access_token)

        if not named_locations:
            log.result_message(ResultLevel.INFO, f"No Conditional Access Named Location found for [{location_identifier}]")
            return

        if len(named_locations) == 1:
            log.result_data({"named_locations": named_locations[0]})
        else:
            log.result_data({"named_locations": named_locations})

        details = "\n".join(f"- {loc['name']} ({loc['id']})" for loc in named_locations)
        log.result_message(ResultLevel.SUCCESS, f"Found Conditional Access Named Location(s):\n{details}")
