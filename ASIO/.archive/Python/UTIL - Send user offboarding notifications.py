...
data_to_log = {}
bot_name = "UTIL - Send user offboarding notifications"
...
def format_date(date_raw):
    try:
        parsed = datetime.strptime(date_raw, "%Y-%m-%dT%H:%M")
...
def main():
    try:
        ...
        tenant_id, access_token = get_graph_token(log, http_client, vault_name, "MIT")
        if not isinstance(access_token, str) or "." not in access_token:
            record_result(log, ResultLevel.WARNING, "MIT Graph token is malformed")
            return
        ...
        if operation == "Complete":
            ...
            record_result(log, ResultLevel.INFO, f"No matching logic for operation [{operation}]")
            return

        if send_email(log, http_client, msgraph_base_url, access_token, sender_email, recipient_emails, subject, html_body):
            record_result(log, ResultLevel.SUCCESS, f"Sent user offboarding [{operation}] notification to [{recipient_emails}]")
