...
def main():
    try:
        try:
            input = Input()
            user_identifier = input.get_value("User_1743425589488").strip()
        except Exception:
        ...
        if not user_email:
            ...
            return
        if not user_sam:
            record_result(log, ResultLevel.WARNING, f"No SAM account name found for [{user_identifier}] (cloud-only user)")
            return
