...
def main():
    try:
        try:
            ...
            raw_guide = input.get_value("Guides_1749635761818")
        except Exception:
        ...
        selected_guide = raw_guide.strip() if raw_guide else ""

        if not recipient_emails or not selected_guide:
            record_result(log, ResultLevel.WARNING, "Bot completed with no recipient or notification selected. No action taken")
            return

        log.info(f"Notification selected: {selected_guide}")
        ...
        if selected_guide == "Application Control":
            subject, html_body = generate_mit_application_control()
        elif selected_guide == "Email Signature Management":
            subject, html_body = generate_mit_email_signature_management()
        elif selected_guide == "Company Portal":
            subject, html_body = generate_mit_company_portal()
        elif selected_guide == "Password Management":
            subject, html_body = generate_mit_password_manager()
        else:
            record_result(log, ResultLevel.WARNING, f"Unrecognized notification type: {selected_guide}")
            return

        if send_email(log, http_client, msgraph_base_url, access_token, sender_email, recipient_emails, subject, html_body):
            record_result(log, ResultLevel.SUCCESS, f"Successfully sent {selected_guide} notification to [{recipient_emails}]")
