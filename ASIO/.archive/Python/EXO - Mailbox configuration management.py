...
def update_mailbox_regional_configuration(log, user_identifier, time_zone, language="en-US", date_format="dd/MM/yyyy", time_format="HH:mm", azure_domain, token):
    log.info(f"Updating regional configuration for user [{user_identifier}]")
    ...
    try {{
        Set-MailboxRegionalConfiguration -Identity ${user_identifier} -Language '{language}' -TimeZone '{time_zone}' -DateFormat '{date_format}' -TimeFormat '{time_format}'
    }} catch {{
        Write-Output "Unable to set mailbox regional configuration"
        Write-Error "Error: $_"
    }}
...
def main():
    try:
        try:
            user_identifier = input.get_value("UserPrincipalName_1755842965828")
            ticket_number = input.get_value("TicketNumber_1755842964935")
            auth_code = input.get_value("AuthCode_1755843143845")
            provided_token = input.get_value("ProvidedToken_1755843133515")
            operation = input.get_value("Operation_1755843092522")
            language = input.get_value("Language_1755842966871")
            time_zone = input.get_value("TimeZone_1755842967673")
            date_format = input.get_value("DateFormat_1755842968588")
            time_format = input.get_value("TimeFormat_1755843171235")
        except Exception:
        ...
        if operation == "Update mailbox regional configuration":
            success, output = update_mailbox_regional_configuration(log, user_identifier, time_zone, language, date_format, time_format, azure_domain, exo_access_token)
            if not success:
                record_result(log, ResultLevel.WARNING, f"Failed to update mailbox regional configuration for [{user_identifier}]")
                return
            record_result(log, ResultLevel.SUCCESS, f"Successfully updated mailbox regional configuration for [{user_identifier}]")
